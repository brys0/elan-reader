...
>
    <button
        class={cn(
            ...
            scanning
                ? `${shadows.enroll.active} animate-pulse duration-5000`
                : error
                  ? `${shadows.error.active} animate-pulse duration-5000`
                  : shadows.enroll.base,
        )}
        onclick={enroll}
...
<div class="flex flex-col w-full gap-4 justify-center items-center">
    {#if error}
        <div
            transition:fly={{ y: -20 }}
            class="py-1.5 px-2 text-center select-none rounded-xl drop-shadow-lg w-full text-sm font-mono text-red-500"
        >
            {error}
        </div>
    {/if}
        ...
        <Tooltip.Provider>
            <Tooltip.Root delayDuration={0} disableCloseOnTriggerClick>
                ...
                <Tooltip.Content class="text-sm text-center w-fit max-w-60">
                    More scans can improve accuracy of the fingerprint
                    recognition.
                </Tooltip.Content>
            </Tooltip.Root>
        </Tooltip.Provider>
