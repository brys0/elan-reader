...
<Drawer.Root bind:open={$switcherModal} shouldScaleBackground setBackgroundColorOnScale={false}>
        ...
        <Drawer.Footer class="overflow-auto flex flex-col gap-6 my-6">
            ...
            <div class="flex flex-col gap-1 w-full">
                {#each scanners as scanner}
                    <button onclick={() => changeScanner(scanner)} class={cn(
                        ...
                        selected === scanner && "bg-secondary/40 fill-primary"
                    )}>
                        <div class="flex-grow inline-flex items-center gap-3 text-muted-foreground">
                            <Devices size={22} class="fill-inherit" />
                            <p class="text-sm text-primary">{scanner}</p>
                        </div>
                        {#if selected === scanner}
                            <Check size={18} weight="bold" class="fill-inherit" />
                        {/if}
                    </button>
                {/each}                
            </div>
